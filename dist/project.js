...
const composer_1 = require("./bundler/composer");
class Project {
    ...
    async load() {
        try {
            if (this.composer) {
                this.config.preload(Object.assign({ name: this.composerData.name, version: this.composerData.version, author: this.getAuthorString(this.composerData.authors), namespaces: [] }, this.composerData.extra.packr));
            }
    ...
    getNamespaces() {
        if (!this.namespaces) {
            ...
            for (let prefix in composer0)
                mappedNamespaces[prefix] = composer0[prefix];
            // Add PSR-4 namespaces from composer
            let composer4 = this.getComposer().getNamespacesPsr4();
            for (let prefix in composer4)
                mappedNamespaces[prefix] = composer4[prefix];
    ...
    getComposer() {
        if (!this.composerReader) {
            this.composerReader = new composer_1.Composer(this.directoryPath);
        }
