...
async function info(args) {
    if (!args[0])
        return console.log('Usage: packr info <file>');
    ...
    let numberBits = buildInfo.bits.length;
    let files = [];
...
function drawValue(name, value) {
    console.log(chalk_1.default.green(name + ': ') + ' '.repeat(15 - name.length), value);
}
function getFileSize(bytes) {
    if (bytes >= 1024 * 512)
        return (Math.ceil(100 * (bytes / 1048576)) / 100) + ' MiB';
    if (bytes >= 1024)
        return (Math.ceil(100 * (bytes / 1024)) / 100) + ' KiB';
    return bytes + ' bytes';
}
